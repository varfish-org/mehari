...
/// Code for transparently reading gnomAD exomes/genomes data
mod gnomad_nuclear {
    use super::GenomeRelease;

    /// Guess genome release from gnomAD autosomal VCF files.
    ///
    /// Returns an `Err` if guessing failed.
    pub fn guess_genome_release(path: &str) -> Result<GenomeRelease, anyhow::Error> {
...
/// Number of samples in Helix.
const HELIX_AC: u32 = 196554;

/// Record used for parsing the HelixMtDb TSV file.
